...
function init(server) {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on(
      "setMotorSpeed",
      (jarName, deviceName, deviceGroup, newSpeed, callback) => {
    ...
    socket.on(
      "toggleValve",
      (jarName, deviceName, deviceGroup, state, callback) => {
    ...
    socket.on("setSensor", (jarName, deviceGroup, newValue, callback) => {
      ...
      switch (deviceGroup) {
        case "tempProbe":
          console.log(
            "received new value for tempProbe of ",
            jarName,
            ": ",
            newValue
          );
...
function checkCallback(callback, socketID, functionName) {
  if (typeof callback !== "function") {
    console.log(
      socketID + " triggered " + functionName + " without providing a callback"
    );
    return (_) => {};
