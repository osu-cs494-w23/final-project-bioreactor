const emptyJars = Array.from(["Jar 1", "Jar 2", "Jar 3"], name => {
    return {
        ...
        "state": "STOPPED" // states should be "RUNNING", "PAUSED", and "STOPPED", i don't want to deal with JS's janky enums yet
    }
})
...
export function bioreactorReducer(app = {"jars": emptyJars}, action) {
    switch (action.type) {
        ...
        case "JAR_CHANGE_FIELD": {
            return {
                ...app,
                "jars": mapNewJarField(app["jars"], action["jarName"], action["field"], action["newValue"])
            }
        }
        default:
            return app
